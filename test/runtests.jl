...
- convert all numbers to float in arrays where at least one number is a float
- removal of comments
- keys in packets have apostrophes
- removal of trailing commas before closing bracket
    ...
    @testset "CZML Circles and Ellipses" begin
        ...
        expected_result = """[
            ...
            },
            {
              ...
              "position": {
                "cartographicDegrees": [-103.0, 40.0, 0.0]
              },
              "ellipse": {
        ...
        @test JSON.parse(expected_result) == JSON.parsefile(fileName)
    end
